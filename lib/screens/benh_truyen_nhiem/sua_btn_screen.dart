...
class _SuaBTNScreenState extends State<SuaBTNScreen> {
  ...
  // ── Dropdown values — dùng CategoryItem để lưu {id, name} ──────────────
  ...
  void initState() {
    ...
    // Load CategoryItem từ model — tương thích với dữ liệu cũ (string tên) lẫn mới (Map id/name)
    _gioiTinh        = b.gioiTinhItem;
    _danToc          = b.danTocItem;
    _ngheNghiep      = b.ngheNghiepItem;
    _coThai          = b.coThaiItem;
    _cityIdHoc       = b.cityIdHocItem;
    _wardIdHoc       = b.wardIdHoc;
    _cityId          = b.cityIdItem;
    _wardId          = b.wardId;
    _coSoDieuTri     = b.coSoDieuTriItem;
    _cityIdCSDT      = b.cityIdCSDTItem;
    _hinhThucDieuTri = b.hinhThucDieuTriItem;
    _chanDoanBenh    = b.chanDoanBenhItem;
    _phanDoBenh      = b.phanDoBenhItem;
    _thongTinDieuTri = b.thongTinDieuTriItem;
    _benhNenKemTheo  = b.benhNenKemTheoItem;
    _phanLoaiChanDoan= b.phanLoaiChanDoanItem;
    _layMauXN        = b.layMauXNItem;
    _loaiBenhPham    = b.loaiBenhPhamItem;
    _loaiXN          = b.loaiXNItem;
    _ketQuaXN        = b.ketQuaXNItem;
    _tinhTrangTiem   = b.tinhTrangTiemItem;
  }
  ...
  void dispose() {
    for (final c in [
      ...
      _noiOHienNayCtrl, _khuPhoApCtrl, _soHSBACtrl,
      _chanDoanBienChungCtrl, _chanDoanKemTheoCtrl,
      _ngayKhoiPhatCtrl, _ngayNhapVienCtrl, _ngayXVTVCVCtrl,
  ...
  Future<void> _pickDate(TextEditingController ctrl, {bool withTime = false}) async {
    ...
    final d = await showDatePicker(
      ...
      firstDate: DateTime(1900), lastDate: DateTime(now.year + 1),
    );
    if (d == null) return;
  ...
  Future<void> _luu() async {
    ...
    try {
      final updated = BenhTruyenNhiem(
        ...
        benhAnId: widget.benhAn.benhAnId,
          ...
          children: [
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Row(children: [
                  const Icon(Icons.badge, color: Color(0xFF2E7D32), size: 20),
            ...
            _Sec('Thông tin cá nhân (2–15)', Icons.person_outline, [
              ...
              _dd('4. Giới tính', Icons.wc_outlined, _gioiTinh, CategoryOptions.gioiTinh,
                  (v) => setState(() => _gioiTinh = v)),
              _dd('5. Dân tộc', Icons.people_outline, _danToc, CategoryOptions.danToc,
                  (v) => setState(() => _danToc = v)),
              _tf(_maDinhDanhCtrl, '6. Mã định danh (CCCD/000)', Icons.credit_card_outlined,
              ...
              _dd('9. Có thai', Icons.pregnant_woman_outlined, _coThai, CategoryOptions.coKhong,
                  (v) => setState(() => _coThai = v)),
              _tf(_tuanThaiCtrl, '10. Tuần thai', Icons.access_time_outlined, type: TextInputType.number),
              _dd('11. Nghề nghiệp', Icons.work_outline, _ngheNghiep, CategoryOptions.ngheNghiep,
                  (v) => setState(() => _ngheNghiep = v)),
              ...
              _dd('14. Tỉnh nơi làm/học', Icons.map_outlined, _cityIdHoc, CategoryOptions.tinh,
                  (v) => setState(() => _cityIdHoc = v)),
            ]),
            const SizedBox(height: 12),

            _Sec('Địa chỉ hiện tại (16–20)', Icons.home_outlined, [
              _tf(_noiOHienNayCtrl, '16. Nơi ở hiện nay', Icons.home_outlined),
              _dd('17. Tỉnh nơi ở', Icons.map_outlined, _cityId, CategoryOptions.tinh,
            ...
            _Sec('Điều trị & Chẩn đoán (21–33)', Icons.local_hospital_outlined, [
              _dd('21. Cơ sở điều trị', Icons.local_hospital_outlined, _coSoDieuTri,
                  CategoryOptions.coSoDieuTri, (v) => setState(() => _coSoDieuTri = v)),
              _dd('22. Tỉnh cơ sở điều trị', Icons.map_outlined, _cityIdCSDT, CategoryOptions.tinh,
                  (v) => setState(() => _cityIdCSDT = v)),
              _dd('23. Hình thức điều trị', Icons.health_and_safety_outlined, _hinhThucDieuTri,
                  CategoryOptions.hinhThucDieuTri, (v) => setState(() => _hinhThucDieuTri = v)),
              _dd('24. Chẩn đoán bệnh', Icons.content_paste_search_outlined, _chanDoanBenh,
                  CategoryOptions.chanDoanBenh, (v) => setState(() => _chanDoanBenh = v)),
              _dd('25. Phân độ bệnh', Icons.bar_chart_outlined, _phanDoBenh,
                  CategoryOptions.phanDoBenh, (v) => setState(() => _phanDoBenh = v)),
              _dd('26. Thông tin điều trị', Icons.medical_services_outlined, _thongTinDieuTri,
                  CategoryOptions.dieuTri, (v) => setState(() => _thongTinDieuTri = v)),
              ...
              _dd('29. Bệnh nền kèm theo', Icons.monitor_heart_outlined, _benhNenKemTheo,
                  CategoryOptions.benhNen, (v) => setState(() => _benhNenKemTheo = v)),
              ...
              _dd('33. Phân loại chẩn đoán', Icons.category_outlined, _phanLoaiChanDoan,
                  CategoryOptions.phanLoaiChanDoan, (v) => setState(() => _phanLoaiChanDoan = v)),
            ]),
            const SizedBox(height: 12),

            _Sec('Xét nghiệm (34–39)', Icons.biotech_outlined, [
              _dd('34. Lấy mẫu XN', Icons.science_outlined, _layMauXN, CategoryOptions.coKhong,
                  (v) => setState(() => _layMauXN = v)),
              _dd('35. Loại bệnh phẩm', Icons.water_drop_outlined, _loaiBenhPham,
                  CategoryOptions.loaiBenhPham, (v) => setState(() => _loaiBenhPham = v)),
              ...
              _dd('38. Loại xét nghiệm', Icons.manage_search_outlined, _loaiXN,
                  CategoryOptions.loaiXetNghiem, (v) => setState(() => _loaiXN = v)),
              _dd('39. Kết quả XN', Icons.assignment_turned_in_outlined, _ketQuaXN,
                  CategoryOptions.ketQuaXetNghiem, (v) => setState(() => _ketQuaXN = v)),
            ]),
            const SizedBox(height: 12),

            _Sec('Tiêm chủng (40–41)', Icons.vaccines_outlined, [
              _dd('40. Tình trạng tiêm', Icons.vaccines_outlined, _tinhTrangTiem,
                  CategoryOptions.tinhTrangTiemChung, (v) => setState(() => _tinhTrangTiem = v)),
              _tf(_soMuiTiemCtrl, '41. Số mũi tiêm/uống', Icons.format_list_numbered_outlined,
                  type: TextInputType.number),
            ]),
            const SizedBox(height: 12),
            ...
            _Sec('Dịch tễ (42–46)', Icons.travel_explore_outlined, [
              ...
              _tf(_emailDieuTraCtrl, '46. Email đơn vị điều tra', Icons.email_outlined,
            ...
            _Sec('Báo cáo (47–51)', Icons.summarize_outlined, [
              ...
              _tf(_emailBaoCaoCtrl, '50. Email người báo cáo', Icons.email_outlined,
                  type: TextInputType.emailAddress),
  ...
  InputDecoration _dec(String label, IconData icon, {IconData? suffix}) =>
      InputDecoration(
        labelText: label,
